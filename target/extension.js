...
import * as HistoryTracker from 'history_tracker';
const LastPassClient = Me.imports.lastpass.client.LastPassClient;
import * as from from LastPassClient;
from;
'lastpass/client';
const ICON_NAME = 'channel-secure-symbolic';
const enableHistory = true;
var LastPassButton = GObject.registerClass(class LastPassButton extends PanelMenu.Button {
    _init() {
    ...
    _openVault(cacheWhileMenuOpen, username = '', errorMessage = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let credentials;
            if (vault == null) {
                credentials = yield ModalLoginDialog.prompt({ initialUsername: username, errorMessage: errorMessage });
                if (username != credentials.username) {
                    this._settings.set_string('last-username', credentials.username);
                }
                try {
                    vault = yield this._client.getVault(credentials.username, credentials.password);
                    this._vault = vault;
                    yield this._saveVault();
                }
                catch (e) {
                ...
                credentials = yield ModalLoginDialog.prompt({ errorMessage: errorMessage, reprompt: true });
            }
            try {
                accounts = yield vault.open(credentials.password);
            }
            catch (e) {
    ...
    _saveVault() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                yield this._vault.write(vaultFile);
            }
            catch (e) {
    ...
    _readVault() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (vaultFile.query_exists(null)) {
                this._vault = yield this._client.readVaultFromFile(vaultFile);
            }
        });
    }
    ...
    _bindShortcuts() {
        Main.wm.addKeybinding('toggle-lastpass-menu', this._settings, Meta.KeyBindingFlags.NONE, Shell.ActionMode.ALL, () => this.menu.toggle());
    }
    _unBindShortcuts() {
...
class ModalLoginDialog extends ModalDialog.ModalDialog {
    ...
    _doneLogin() {
        ...
        this.close();
        this.emit('login', this._reprompt ? '' : this._usernameEntry.get_text(), this._passwordEntry.get_text(), false);
    }
}
let lastPassButton;
function enable() {
